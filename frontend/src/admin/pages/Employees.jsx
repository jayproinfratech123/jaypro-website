...
export default function Employees() {
  ...
  return <section className="admin-settings"><h2>Employees</h2><p>Create employee credentials, then assign leads using the eye icon in All Leads. Employees sign in at /employee/login.</p>
    <form onSubmit={submit}><label>Employee name<input name="name" required maxLength={150}/></label><label>Login ID<input name="loginId" required minLength={3} maxLength={254} pattern="[a-zA-Z0-9][a-zA-Z0-9._@\-]+" autoComplete="off"/></label><label>Password<input name="password" type="password" minLength={12} autoComplete="new-password" placeholder="Leave blank to generate a password"/></label><button className="add-lead-btn" disabled={busy}>{busy?'Creating...':'Add employee'}</button></form>
    {error&&<p role="alert">{error}</p>}{created&&<div role="status"><h3>Employee created — save these credentials</h3><p>Login ID: <strong>{created.employee.loginId}</strong></p><p>Password: <code>{created.password}</code></p><p>The password is shown only now. Share it privately with this employee.</p><button type="button" onClick={()=>setCreated(null)}>Hide credentials</button></div>}
    <div className="table-container"><table className="lead-table"><thead><tr><th>Name</th><th>Login ID</th></tr></thead><tbody>{employees.map(e=><tr key={e.id}><td>{e.name}</td><td>{e.loginId}</td></tr>)}</tbody></table></div>
  </section>;
}
